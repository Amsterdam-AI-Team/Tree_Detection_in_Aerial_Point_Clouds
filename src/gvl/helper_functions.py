...
def get_tilecode_from_filename(filename, n_digits=3):
    ...
    return re.match(rf".*(\d{{{n_digits}}}_\d{{{n_digits}}}).*", filename)[1]


def get_bbox_from_tile_code(tile_code, padding=0, width=DEFAULT_BOX_SIZE, height=DEFAULT_BOX_SIZE):
    """
    ...
    return (
        (x_min - padding, y_min + height + padding),
...
def process_ahn_las_tile(ahn_las_file, out_folder="", resolution=0.1):
    ...
    grid_y, grid_x = np.mgrid[
        y_max - resolution / 2 : y_min : -resolution, x_min + resolution / 2 : x_max : resolution
    ]

    # Methods for generating surfaces (grids)
    ground_surface = ahn_preprocessing._get_ahn_surface(
        ahn_las, grid_x, grid_y, "idw", ahn_preprocessing.AHN_GROUND
    )
    building_surface = ahn_preprocessing._get_ahn_surface(
        ahn_las, grid_x, grid_y, "max", ahn_preprocessing.AHN_BUILDING, max_dist=1.5
    )

    filename = os.path.join(out_folder, "ahn_" + tile_code + ".npz")
    np.savez_compressed(
        filename, x=grid_x[0, :], y=grid_y[:, 0], ground=ground_surface, building=building_surface
    )
    return filename
...
def calculate_normals(points_xyz):
    ...
    points = np.stack((points_xyz[:, 0], points_xyz[:, 1], points_xyz[:, 2]), axis=-1)
...
def voxel_downsample(points_xyz, voxel_size):
    ...
    if ndims == 2:
        points_xyz = np.stack(
            (points_xyz[:, 0], points_xyz[:, 1], np.zeros_like(points_xyz[:, 0])), axis=-1
        )
